...
class Cost_Node(Node):
    def __init__(self):
        ...
        # class定義
        self.roughness = roughness_cost(self.cell_res, self.get_logger())
        self.slopecost = slope_cul(self.cell_res, self.get_logger())
        self.grid_manege = grid_management(self.cell_res)
    ...
    def callback(self, scan):
        ...
        points = self.grid_manege.get_point(scan, self.pose.x, self.pose.y, self.pose.yaw)
    ...
    def odomcallback(self, odom):
        self.pose.x = odom.pose.x
        self.pose.y = odom.pose.y
        self.pose.get_yaw(odom.quatanion.x, odom.quatanion.y, odom.quatanion.z, odom.quatanion.w)
    ...
    def publish_grid(self):
        ...
        now_i = int(self.pose.x / self.cell_res)
        now_j = int(self.pose.y / self.cell_res)
        ...
        for i in range(self.grid_size):
            for j in range(self.grid_size):
                non_deta = 0
                key = (i + now_i, j + now_j)
                data = np.sqrt(self.grid_var[key])
                if data > 10 :
                    data = 10
                marker = Marker()
